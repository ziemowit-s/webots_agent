...
class BrianLIFAgent(BrianAgent):
    ...
    def _make_network(self):
        ...
        """
        self.inp = NeuronGroup(self.input_size, model=neuron, method='euler', threshold='v>0.1',
                               reset='v=0', refractory=10 * ms, namespace={'tau': 10*ms})
        self.network.add(self.inp)
        self.hid = NeuronGroup(self.hidden_size, model=neuron, method='euler', threshold='v>0.1',
                               reset='v=0', refractory=10 * ms, namespace={'tau': 10*ms})
        self.network.add(self.hid)
        self.output = NeuronGroup(self.output_size, model=neuron, method='euler', threshold='v>0.1',
                                  reset='v=0', refractory=10 * ms, namespace={'tau': 10*ms})
        self.network.add(self.output)
